...
def word_wrap(image, ctx, text, roi_width, roi_height, file_disting_num, file_disting_num2):
    ...
    def eval_metrics(txt):
        ...
        try:
            ...
            rename(originalfont, originalfont[:(originalfont.rfind("\\")+1)] + "foo" + str(file_disting_num) +',' + str(file_disting_num2) + originalfont[-4:])
            ctx.font = originalfont[:(originalfont.rfind("\\")+1)] + "foo" + str(file_disting_num) +',' + str(file_disting_num2) + originalfont[-4:]
            metrics = ctx.get_font_metrics(image, txt, False)
...
def gen_font_image_letter(font, letter, file_disting_num, file_disting_num2, letter_size = 32, img_path = 'C:\\Users\\remake\\gdrive\\remake\\font-compat-test-service\\data\\image\\letter_display_for_ver3.png'):
    ...
    with wand_image(filename=img_path) as img:
        with Drawing() as ctx:
            ...
            mutable_message = word_wrap(img,
                                        ...
                                        letter,
                                        ROI_WIDTH,
                                        ROI_HEIGHT, file_disting_num, file_disting_num2)
            # print("후")
            # print(ctx)
            ...
            try:
                ...
                rename(font[:(font.rfind("\\")+1)] + "foo" + str(file_disting_num) +','+ str(file_disting_num2) + font[-4:], font)
...
def gen_font_image(font, message, file_disting_num, letter_size = 32, img_path = 'C:\\Users\\remake\\gdrive\\remake\\font-compat-test-service\\data\\image\\letter_display_for_ver3.png'):
    font_copy = font
    letter_img_list = []    
    ...
    for i in range(len(message)):
        letter_img_list.append(gen_font_image_letter(font, message[i], file_disting_num, i))
    ...
    imwrite("C:\\Users\\remake\\gdrive\\remake\\font-compat-test-service\\frontend\\font-compat-test-service\\result\\" + font_copy[(font_copy.rfind("\\")+1):-4]+".png", font_img)
...
def main():
    ...
    Parallel(n_jobs=4)(delayed(gen_font_image)(font, input_message, index) for index, font in enumerate(kor_font_file_list))
        # print(font)
